...
class ResidualConnection(nn.Module):
    ...
    def forward(self, x, sublayer):
        # x: current input embeddings, sublayer: muti head attention
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, src_mask):
        x = self.residual_connections[0](x, lambda x: self.self_attention_block(x, x, x, src_mask))  # lambda表达式，表示接受输入x，返回函数计算结果
...
class DecoderBlock(nn.Module):
    def __init__(self, self_attention_block: MutiHeadAttentionBlock, cross_attention_block: MutiHeadAttentionBlock,
...
class ProjectionLayer(nn.Module):
    ...
    def forward(self, x):
        # (batch, seq_lan, d_model) --> (batch, seq_len, vocab_size)
        return torch.log_softmax(self.proj(x), dim=-1)


class Transformer(nn.Module):
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: InputEmbeddings, tgt_embed: InputEmbeddings, src_pos: PostionalEncoding,
                 tgt_pos: PostionalEncoding, projection_layer: ProjectionLayer):
        super().__init__()
